    ...
    if(isset($_POST['edit']))
    {
        ...
        if ($_FILES['file']['size'] > 0) {
...

            {
                ...
                mysqli_query($connection, "UPDATE kuliner SET kulinerNAMA = '$kulinerNAMA', kulinerGAMBAR = '$namafile' 
                WHERE kulinerKODE = '$kulinerKODE'");
                
            }
        } else {
            mysqli_query($connection, "UPDATE kuliner SET kulinerNAMA = '$kulinerNAMA' WHERE kulinerKODE = '$kulinerKODE'");
           }
    }

    $kodeditkuliner = $_GET["ubah"];
    $editgambar = mysqli_query($connection, "SELECT * FROM kuliner
    WHERE kulinerKODE = '$kodeditkuliner'");
    $roweditfoto = mysqli_fetch_array($editgambar);
?>
    ...
    <div class="row">
        ...
        <div class="col-sm-10">
            <div class="jumbotron jumbotron-fluid">
                <div class="container">
                    <h1 class="display-4">Edit Gambar Makanan</h1>
                </div>
            </div>
...
<form method="POST" enctype="multipart/form-data">
    <div class="form-group row">
        <label for="kulinerKODE" class="col-sm-2 col-form-label">Kode Makanan</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" id="kulinerKODE" name="inputkode" value="<?php echo $roweditfoto['kulinerKODE']; ?>" readonly>
            </div>
    </div>
    ...
    <div class="form-group row">
        <label for="kulinerNAMA" class="col-sm-2 col-form-label">Nama Makanan</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" id="kulinerNAMA" name="inputnama" value="<?php echo $roweditfoto['kulinerNAMA']; ?>">
            </div>
    </div>
...
<?php 
    if (!empty($row['kulinerGAMBAR']) && is_file("images/".$row['kulinerGAMBAR'])): ?>
        <img src="images/<?php echo $row['ulinerGAMBAR']?>" width="80" alt="Current Image">
    <?php else: ?>
    <?php endif; ?>
</td>
...
<div class="row">
    <div class="col-sm-1"></div>
        <div class="col-sm-10">
            <div class="jumbotron jumbotron-fluid">
                <div class="container">
                    <h1 class="display-4">Daftar Foto Makanan</h1>
                </div>
            </div>
...
<tr>
    ...
    <td>
        <?php 
            if(is_file("images/".$row['kulinerGAMBAR']))
            { ?>
